...
# Humam Rashid
#
# csv_join: A join implementation for CSV files. Assumes both CSV files have same number of fields
# per record and the join key is at the same position in each record (if it exists). This 
# implementation is essentially a rudimentary version of the UNIX join utility specific to CSV
# files. In keeping with the output of the UNIX join, the join key is printed on the leftmost column
...
#   records can be sorted (for merge joins).
#
#   3.) <file1> <file2>, CSV files to join.

require 'csv'

# Adding methods to String class to check if a String value is representing an integer or float.
class String
  def int?
...
# Checks if a string field value is less than another according to type represented (string, int or
# float).
...
# Checks if a string field value is greater than another according to type represented (string, int
# or float).
